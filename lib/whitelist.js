...
Whitelist.prototype.set = function(domains) {
  if (!domains) {
    ...
    if (!this._domains) {
        ...
        this._domains = domains.reduce(function(o, x) {
                      o[x] = true; return o; }, this._domains);
    }
...
Whitelist.prototype.mergeWithStoredDomains = function() {
    var tmpDomains = {};
    for (var attrname in this._domains) { tmpDomains[attrname] = this._domains[attrname]; }
    for (var attrname in storedWhitelist) { tmpDomains[attrname] = storedWhitelist[attrname]; }
    
    this._domains = tmpDomains;
};
